...
class Strategy_MACD_01(CtaTemplate):
    ...
    def onBarM15(self, bar):
        ...
        jincha_15f = self.lineM15.lineDif[-1 - idx] < self.lineM15.lineDea[-1 - idx] \
                     ...
                     and abs(self.lineM15.lineMacd[0 - idx]) >= 0.5
        ...
        if self.position.pos == 0:
            ...
            if jincha_15f:
                ...
                if max(difdea) >= 8:  # 高位金叉，不开多仓
                    return

                if max(difdea) <= -8:  # 低位金叉，开重仓
...
def testRbByBar():
    ...
    # engine.setDatabase(dbName='stockcn', symbol='rb')
    engine.setDatabase(dbName='stockcn', symbol='I')
    ...
    # 开始跑回测
    engine.runBackTestingWithBarFile(os.getcwd() + '/cache/I99_20100101_20170707_15m.csv')
