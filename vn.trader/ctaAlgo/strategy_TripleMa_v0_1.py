...
from vtConstant import EMPTY_STRING, EMPTY_INT, DIRECTION_LONG, DIRECTION_SHORT, OFFSET_OPEN, OFFSET_CLOSE, \
    OFFSET_CLOSETODAY, OFFSET_CLOSEYESTERDAY, STATUS_CANCELLED
...
class Strategy_TripleMa(CtaTemplate):
    ...
    def __init__(self, ctaEngine, setting=None):
        ...
        if setting:
            ...
            # 这里的mode考虑的主要是tick的模式还是分钟的模式
            try:
                mode = setting['mode']  # setting['mode']在下面onbar和ontick中设置
                if mode != EMPTY_STRING:
                    self.lineM5.setMode(setting['mode'])
            except KeyError:
                self.lineM5.setMode(self.lineM5.TICK_MODE)

        self.onInit()  # 自动加载策略而不需要再在界面上点击初始化按钮
    ...
    def onStop(self):
        """停止策略（必须由用户继承实现）"""
        self.uncompletedOrders.clear()  # 将当前所有没有完成的order全部清除
        self.pos = EMPTY_INT
    ...
    def onTick(self, tick):
        ...
        if not (self.inited and len(self.lineM5.lineMa3) > 0):
            # 当lineMa3这条最长的均线有了，其他均线也会有，那么也就可以开始做逻辑判断
            return
    ...
    def onBarM5(self, bar):
        ...
        # 这样设置方便区分实盘和回测，因为实盘的时候-2的bar是确定的，而回测的时候-1的bar是确定的
        ...
        # 如果未持仓，检查是否符合开仓逻辑
        if self.pos == 0:
            # MA10上穿MA20，即金叉，同时 MA10 > MA120， bar.close > MA120
            # 在120日均线上，出现金叉做多
            # 即当idx=1的时候是处于回测的时候，即-1和-2的比较
            # 而当idx=2的时候是处于实盘运行的时候，即-2和-3的比较
            if self.lineM5.lineMa1[-1 - idx] < self.lineM5.lineMa2[-1 - idx] \
                ...
                orderid = self.buy(price=bar.close, volume=self.inputSS, orderTime=self.curDateTime)  # 买多
                if orderid:
                    self.lastOrderTime = self.curDateTime
                return

            # MA10下穿MA20，即死叉，同时MA10 < MA120， bar.close < MA120
            # 在120日均线下方的时候，出现死叉做空
            if self.lineM5.lineMa1[-1 - idx] > self.lineM5.lineMa2[-1 - idx] \
                    and self.lineM5.lineMa1[0 - idx] < self.lineM5.lineMa2[0 - idx] \
                    and self.lineM5.lineMa1[0 - idx] < self.lineM5.lineMa3[0 - idx] \
                    and bar.close < self.lineM5.lineMa3[-1]:
                self.writeCtaLog(u'{0},开仓空单{1}手,价格:{2}'.format(bar.datetime, self.inputSS, bar.close))  # %字符串转换
                orderid = self.short(price=bar.close, volume=self.inputSS, orderTime=self.curDateTime)  # 卖空
                if orderid:
                    self.lastOrderTime = self.curDateTime  # 记录委托时间即为现在的时间
                return
...
def testRbByTick():
    ...
    # 设置引擎的回测模式为Tick
    engine.setBacktestingMode(engine.TICK_MODE)  # 数据模式设置
...
def testRbByBar():
    ...
    # 设置引擎的回测模式为bar
    engine.setBacktestingMode(engine.BAR_MODE)  # 数据模式设置
    ...
    engine.setRate(float(0.0002))  # 万1
    engine.setSize(10)  # 合约大小
