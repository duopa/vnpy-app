...
class Strategy_MACD(CtaTemplate):
    ...
    加仓、减仓：
    减仓：
    当macd出现连续两根缩短的时候，减一半的仓位
    当macd第二次出现连续两根缩短的时候，清仓
    ...
    def __init__(self, ctaEngine, setting=None):
        ...
        self.percent=BacktestingEngine(self)
        self.percent.percent=EMPTY_FLOAT
        ...
        if setting:
            ...
            lineM60Setting = {}
            lineM60Setting['name'] = u'M15'  # k线名称
            lineM60Setting['barTimeInterval'] = 60 * 15  # K线的Bar时长
            ...
            lineM60Setting['minDiff'] = self.minDiff
        ...
        if orderkey in self.uncompletedOrders:
            if order.totalVolume == order.tradedVolume:
                ...
                if self.uncompletedOrders[orderkey]['DIRECTION'] == DIRECTION_LONG and order.offset != OFFSET_OPEN:
                    self.writeCtaLog(u'平空仓完成，原仓位:{0}'.format(self.position.pos))
                ...
                if self.uncompletedOrders[orderkey]['DIRECTION'] == DIRECTION_SHORT and order.offset != OFFSET_OPEN:
                    self.writeCtaLog(u'平多仓完成，原仓位:{0}'.format(self.position.pos))
                    self.position.closePos(direction=DIRECTION_SHORT, vol=order.tradedVolume)
                    self.writeCtaLog(u'新仓位:{0}'.format(self.pos))

                # 开多仓完成
                if self.uncompletedOrders[orderkey]['DIRECTION'] == DIRECTION_LONG and order.offset == OFFSET_OPEN:
                    self.writeCtaLog(u'开多仓完成，原仓位:{0}'.format(self.position.pos))
                ...
                if self.uncompletedOrders[orderkey]['DIRECTION'] == DIRECTION_SHORT and order.offset == OFFSET_OPEN:
                    # 更新仓位
                    self.writeCtaLog(u'开空仓完成，原仓位:{0}'.format(self.position.pos))
                    self.position.openPos(direction=DIRECTION_SHORT, vol=order.tradedVolume, price=order.price)
                    self.writeCtaLog(u'新仓位:{0}'.format(self.position.pos))
    ...
    def onTick(self, tick):
        ...
        if not (self.inited and len(self.lineM15.lineMa3) > 0):
            return
    ...
    def onBarM60(self,bar):
        ...
        if self.lineM60.mode == self.lineM60.TICK_MODE:
            ...
            idx = 1
        Totalpos=self.deposit(bar),
        # 60分钟上金叉+15分钟上金叉，总仓位设置在40%-50%
...
def testRbByBar():
    ...
    engine.setRate(float(0.0002))  # 万3
    engine.setSize(10)  # 合约大小
